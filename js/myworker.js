...
function t(a){var b="/"===a.charAt(0),c="/"===a.substr(-1);(a=hb(a.split("index.html").filter(function(d){return!!d}),!b).join("index.html"))||b||(a=".");a&&c&&(a+="index.html");return(b?"/":"")+a}function ib(a){var b=/^(\/?|)([\s\S]*?)((?:\.{1,2}|[^\/]+?|)(\.[^.\/]*|))(?:[\/]*)$/.exec(a).slice(1);a=b[0];b=b[1];if(!a&&!b)return".";b&&(b=b.substr(0,b.length-1));return a+b}function jb(a){if("/"===a)return"/";a=t(a);a=a.replace(/\/$/,"");var b=a.lastIndexOf("index.html");return-1===b?a:a.substr(b+1)}
...
function pb(){for(var a="",b=!1,c=arguments.length-1;-1<=c&&!b;c--){b=0<=c?arguments[c]:"/";if("string"!=typeof b)throw new TypeError("Arguments to path.resolve must be strings");if(!b)return"";a=b+"/"+a;b="/"===b.charAt(0)}a=hb(a.split("index.html").filter(function(d){return!!d}),!b).join("index.html");return(b?"/":"")+a||"."}var rb=[];function sb(a,b){rb[a]={input:[],output:[],Wa:b};tb(a,ub)}
...
readline(),null!==b&&(b+="\n"));if(!b)return null;a.input=ka(b,!0)}return a.input.shift()},ib:function(a,b){null===b||10===b?(Fa(Sa(a.output,0)),a.output=[]):0!=b&&a.output.push(b)},flush:function(a){a.output&&0<a.output.length&&(Fa(Sa(a.output,0)),a.output=[])}},wb={ib:function(a,b){null===b||10===b?(G(Sa(a.output,0)),a.output=[]):0!=b&&a.output.push(b)},flush:function(a){a.output&&0<a.output.length&&(G(Sa(a.output,0)),a.output=[])}},S={Oa:null,Pa:function(){return S.createNode(null,"index.html",16895,0)},
...
v])}catch(B){throw new Q(29);}u&&(g.node.timestamp=Date.now());return v}});Tb(a,d,f)},Fc,Y={},Qb,Zb;function Hc(a,b,c){if("/"===b[0])return b;if(-100===a)a="index.html";else{a=T[a];if(!a)throw new Q(8);a=a.path}if(0==b.length){if(!c)throw new Q(44);return a}return t(a+"/"+b)}
...
Object.defineProperties(Kb.prototype,{read:{get:function(){return 365===(this.mode&365)},set:function(a){a?this.mode|=365:this.mode&=-366}},write:{get:function(){return 146===(this.mode&146)},set:function(a){a?this.mode|=146:this.mode&=-147}}});Cc();U=Array(4096);Sb(S,"index.html");W("/tmp");W("/home");W("/home/web_user");
...
function onModuleReady(SQL) {
    function createDb(data) {
		...
		// Create additional Oracle like functions:
		db.create_function("ascii",  function (s)    {return s.charCodeAt(0);});
		db.create_function("chr",    function (n)    {return String.fromCharCode(n);}); // same as CHAR(n) in sqlite
		db.create_function("concat", function (s,t)  {return '' + s + t;});
		db.create_function("initcap",function (s)    {return s===null ? s : (''+s).toLowerCase().replace(/(?:^|\s)[a-z]/g, function (m) {return m.toUpperCase(); })});
        db.create_function("lpad",   function (s,n,t=' ') {return s===null ? s : s.padStart(n,t);},-1);
        db.create_function("rpad",   function (s,n,t=' ') {return s === null ? s : s.padEnd(n, t); },-1);
        db.create_function("right",  function (s,n) {return s === null ? s : ('' + s).slice(0, -n); });
        db.create_function("left",   function (s,n) {return s === null ? s : ('' + s).slice(0, n); });
		db.create_function("nvl",    function (x,y) {return x === null ? y : x;});
		db.create_function("translate", 
		          function (s,t1, t2)  {
					 var r = '';
					 for(var j=0;j < s.length;j++ ) {
						 var c = s.charAt(j);
						 var idx = t1.indexOf(c);
						 if(idx>=0) {
							 r+= (idx < t2.length ? t2.charAt(idx) : '');
						 }
						 else {
							 r += c;
						 }
        ...
        db.create_function("add_months", function (date, months) {
            ...
            if (date.indexOf('00:00:00') >= 0) date = date.split('T')[0];
            return date;
        });
        db.create_function("to_number", function (s) { return s === null ? s : s*1; });
        db.create_function("to_date",
            function (s, t) {
                let noTime = false;
                if (s === null || s === '') return null;
                let dt = new Date(s);
                if (Object.prototype.toString.call(dt) != '[object Date]' || isNaN(dt)) return s;
                if (('' + dt).indexOf('00:00:00') >= 0) {
                    noTime = true; // no time portion 
                }
                else {
                    let offset = dt.getTimezoneOffset();
                    dt = new Date(dt.getTime() + (60 * 1000 * offset)); // dt.setTime(dt.getTime()+(60 * 1000 * offset))
                }
                dt = dt.toISOString().replace('T', ' ').slice(0, -1);
                if (noTime) return dt.split(' ')[0];
                return dt.replace('00:00:00.000', '');
            }
        );
        db.create_function("trunc",
            function (x) {
                if (x === null) return x;
                // if numeric, remove decimals, if date - set time to midnight.
                if (typeof x == 'number') return ~~x;
                var dt = new Date(x).toISOString().split('T')[0]; // remove time
                return dt;
            }
        );
        db.create_function("to_char", function (s) { console.log('to_char', s);return s === null ? s : '' + s;});
        db.create_function("to_char",
            function (x, fmt) {
                if (x === null || x === '') return x;
                if (typeof x === 'number') {
                    // Here is where number formatting goes.
                    return x.toLocaleString();
                }
                else {
                    fmt = fmt.replace('A.M.', 'AM').replace('P.M.', 'PM').replace('a.m.', 'am').replace('p.m.', 'pm');
                    let am = 'AM';
                    let dt = new Date(x);
                    if (Object.prototype.toString.call(dt) != '[object Date]' || isNaN(dt)) return x;
                    let offset = dt.getTimezoneOffset();
                    dt = new Date(dt.getTime() + (60 * 1000 * offset)); // dt.setTime(dt.getTime()+(60 * 1000 * offset))
                    dt = '' + dt;
                    let parts = dt.split(' ');
                    console.log('to_char: dt:', dt);
                    if (parts.length > 8) {
                        // it is a date
                        console.log('it is a date:', JSON.stringify(parts));
                        let dy = parts[0]; // short day name
                        let day = { "Sun": "Sunday", "Mon": "Monday", "Tue": "Tuesday", "Wed": "Wednesday", "Thu": "Thursday", "Fri": "Friday", "Sat": "Saturday" }[dy];
                        let d = { "Sun": "1", "Mon": "2", "Tue": "3", "Wed": "4", "Thu": "5", "Fri": "6", "Sat": "7" }[dy];
                        let mon = parts[1];
                        let monthMap = {
                            "Jan": { "mm": 1, "month": "January", "quarter": "1", "roman": "I" },
                            "Feb": { "mm": 2, "month": "February", "quarter": "1", "roman": "II" },
                            "Mar": { "mm": 3, "month": "March", "quarter": "1", "roman": "III" },
                            "Apr": { "mm": 4, "month": "April", "quarter": "2", "roman": "IV" },
                            "May": { "mm": 5, "month": "May", "quarter": "2", "roman": "V" },
                            "Jun": { "mm": 6, "month": "June", "quarter": "2", "roman": "VI" },
                            "Jul": { "mm": 7, "month": "July", "quarter": "3", "roman": "VII" },
                            "Aug": { "mm": 8, "month": "August", "quarter": "3", "roman": "VIII" },
                            "Sep": { "mm": 9, "month": "September", "quarter": "3", "roman": "IX" },
                            "Oct": { "mm": 10, "month": "October", "quarter": "4", "roman": "X" },
                            "Nov": { "mm": 11, "month": "November", "quarter": "4", "roman": "XI" },
                            "Dec": { "mm": 12, "month": "December", "quarter": "4", "roman": "XII" },
                        };
                        let mm = ('0' + monthMap[mon].mm).slice(-2);
                        let month = monthMap[mon].month;
                        let q = monthMap[mon].quarter;
                        let rm = monthMap[mon].roman;
                        let dd = ('0' + parts[2]).slice(-2);
                        let yyyy = parts[3];
                        let yyy = yyyy.slice(-3);
                        let yy = yyyy.slice(-2);
                        let y = yyyy.slice(-1);
                        let mmi = mm * 1;
                        // https://stackoverflow.com/questions/8619879/javascript-calculate-the-day-of-the-year-1-366
                        let ddd = --mmi * 31 - (mmi > 1 ? (1054267675 >> mmi * 3 - 6 & 7) - ((yyyy*1) & 3 || !((yyyy*1) % 25) && (yyyy*1) & 15 ? 0 : 1) : 0) + (dd*1);
                        ddd = '' + ddd; // day of year 1 - 366
                        console.log('ddd', ddd);
                        let hh24 = parts[4].split(':')[0];
                        console.log('hh24-initial', hh24);
                        let hh = (1 * hh24) < 13 ? (hh24 * 1) : (1 * hh24) - 12;
                        console.log('hh24-mid', hh24);
                        if (hh24 === 0) hh = 12;
                        console.log('hh24-final', hh24);
                        hh = ('0' + hh).slice(-2);
                        let mi = parts[4].split(':')[1];
                        let ss = parts[4].split(':')[2];
                        let am = (hh < 12 ? "AM" : "PM");
                        let gmt = parts[5].substr(3); // -0500
                        let tz = parts[6] + ' ' + parts[7] + (parts.length > 8 ? ' ' + parts[8] : '');
                        return fmt
                            .replace('DDD', ddd).replace('ddd', ddd) // day of year 1 - 366
                            .replace('DD', dd).replace('dd', dd)
                            .replace(/[Dd](?!AY|ay)/g, d)  // Day of week, i.e. d = 1,2,3,4,5,6,7
                            .replace('MONTH', month.toUpperCase()).replace('month', month.toLowerCase())
                            .replace('MON', mon.toUpperCase()).replace('mon', mon.toLowerCase())
                            .replace('MM', mm).replace('mm', mm)
                            .replace('RM', rm).replace('rm', rm)
                            .replace('YYYY', yyyy).replace('yyyy', yyyy)
                            .replace('YYY', yyyy).replace('yyy', yyy)
                            .replace('YY', yy).replace('yy', yy)
                            .replace(/\b(?!da|DA)Y/gi, y)
                            .replace('Q', q).replace('q', q)
                            .replace('HH24', hh24).replace('hh24', hh24)
                            .replace('HH12', hh).replace('hh12', hh)
                            .replace('HH', hh).replace('hh', hh)
                            .replace('MI', mi).replace('mi', mi)
                            .replace('SS', ss).replace('ss', ss)
                            .replace('AM', am.toUpperCase()).replace('am', am.toLowerCase())
                            .replace('PM', am.toUpperCase()).replace('am', am.toLowerCase())
                            .replace('DY', dy.toUpperCase()).replace('dy', dy.toLowerCase()).replace('Dy', dy)
                            .replace('DAY', day.toUpperCase()).replace('day', day.toLowerCase()).replace('Day', day)
                            ;
                        // TO DO
                        // YEAR - spells year out, i.e. Twenty Twenty-Two 
