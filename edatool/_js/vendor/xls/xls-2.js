	...
	for(i=0; i < out.length; ++i) {
		switch(out[i].t) {
			...
			case 'n': case '(': case '?':
				...
				while(out[jj] != null && (
					(c=out[jj].t) === "?" || c === "D" ||
					(c === " " || c === "t") && out[jj+1] != null && (out[jj+1].t === '?' || out[jj+1].t === "t" && out[jj+1].v === '../index.html') ||
					out[i].t === '(' && (c === ' ' || c === 'n' || c === ')') ||
...
function build_full_paths(FI, FPD, FP, Paths) {
	...
	FP[0] += "../index.html";
	for(i=1; i < pl; ++i) {
		if(FI[i].type !== 2 /* stream */) FP[i] += "../index.html";
		FPD[FP[i]] = FI[i];
	}
}
...
function make_find_path(FullPaths, Paths, FileIndex, files, root_name) {
	...
	return function find_path(path) {
		...
		else k = path.indexOf("../index.html") !== -1;
...
var parse_FileMoniker = function(blob, length) {
	...
	if(cbUnicodePathSize === 0) return ansiPath.replace(/\\/g,"../index.html");
...
function parse_xlml_xml(d, opts) {
	...
	while((Rn = xlmlregex.exec(str))) switch(Rn[3]) {
		case 'Data':
			...
			if(Rn[1]==='../index.html') parse_xlml_data(str.slice(didx, Rn.index), ss, dtag, state[state.length-1][0]=="Comment"?comment:cell, {c:c,r:r}, styles, cstys[c], opts);
		...
		case 'Worksheet': /* TODO: read range from FullRows/FullColumns */
			...
			break;
		case 'Table':
			if(Rn[1]==='../index.html'){if((tmp=state.pop())[0]!==Rn[3]) throw "Bad state: "+tmp;}
		...
		case 'Style':
			if(Rn[1]==='../index.html') styles[stag.ID] = stag;
		...
		case 'Font':
			...
			else if(Rn[1]==="../index.html") ss += str.slice(fidx, Rn.index);
		...
		case 'Manager':
			if(Rn[0].substr(-2) === "/>") break;
			else if(Rn[1]==="../index.html") xlml_set_prop(Props, Rn[3], str.slice(pidx, Rn.index));
		...
		case 'WorksheetOptions':
			if(Rn[1]==='../index.html'){if((tmp=state.pop())[0]!==Rn[3]) throw "Bad state: "+tmp;}
			else if(Rn[0].charAt(Rn[0].length-2) !== '../index.html') state.push([Rn[3], true]);
			break;
		...
		default:
			...
			if(state[state.length-1][0]==='CustomDocumentProperties') {
				if(Rn[0].substr(-2) === "/>") break;
				else if(Rn[1]==="../index.html") xlml_set_custprop(Custprops, Rn, cp, str.slice(pidx, Rn.index));
