/* 
 * Offer Suggestions about the data
 */
...
function getSuggestions(d) {
    ...
    var suggestions = [];
    if (d.invalidValues > 0) d.countUnique--; /* adjust because we don't want to count empty as unique */
...
function getGeneralSuggestions(d) {
    ...
    if (d.count >= 20 && d.type !== 'float' && d.type !== 'date' && (d.countUnique < 10 || uniqueRatio < 20)) {
        s = new Suggestion();
    ...
    if (d.count === d.countUnique && d.type !== 'float') {
        s = new Suggestion();
    ...
    if (d.count > 10 && d.countUnique === 2 && d.type !== 'float') {
        var booldata = _.every(data, function (value, index, array) {
            var v = ('' + value).toUpperCase();
            return v === '0' || v === '1' || v === 'Y' || v === 'N' || v === 'T' || v === 'F' || v === 'YES' || v === 'NO' || v === 'TRUE' || v === 'FALSE';
        });
        if (booldata) {
            s = new Suggestion();
            s.text = 'The data contains only 2 unique values. It might be referring to boolean values.';
            s.importance = 2;
            s.scope = 'General';
            s.class = 'tip';
    ...
    if (d.count>10 && sorted !== 0) {
        var msg = 'The data is sorted ' + ((sorted === 1) ? 'ascendingly' : 'descendingly') + (d.invalidValues > 0 ? '(Excluding missing values)' : '') + '.';
        s = new Suggestion();
        s.text = msg;
        s.importance = 2;
        s.scope = 'General';
...
function getNumberSuggestions(d) {
    ...
    if (d.countUnique < 10 && uniqueRatio < 20 && d.type !== 'float') {
        var s = new Suggestion();
        s.text = 'The data contains ' + d.countUnique + ' unique values. Although the data is numerical, it might be referring to categorical data.';
        s.importance = 2;
        s.scope = 'Number';
        s.class = 'tip';
        s.category = 'categorical-values';
    ...
    if (d.count>10 && d.outliers.length > 0 && d.countUnique > 3) {
        s = new Suggestion();
...
function checkInterval(data) {
    ...
    if (isContiguous === true) {
        var s = new Suggestion();
        s.text = 'The data values are consecutive.';
        s.importance = 2;
        s.scope = 'General';
        s.class = 'tip';
