...
function MetaReader() {
    ...
    mr.toText = function () {
        ...
        result += 'Generated using Meta Reader (http://www.convertcsv.com/EDATool)\n\n';
        result += mr.description + '\n\n';
        var index = 1;
        _.forEach(mr.statistics, function (col, i) {
            //            console.log(i+'\t'+col)
            result += col.toText(index++);
        });
    ...
    mr.loadFile = function(file, results, config) {
        ...
        if (results) {
            ...
            console.log('... from parseCSVFile');
        } else {
            csv = loadFromFile(file);
            mr.filename = _.last(file.split('../index.html'));
            console.log('... from loadFromFile');
        }
    ...
    var ObjectList = function(data, title, metrics) {
        ...
        self.toText = function (index) {

            var result = '   ' + (index ? index + '. ' : '') + self.title + '\n\n';
            result += '    Column Name: ' + self.columnName + '\n';
            result += '    Data Type: ' + self.type + '\n';
    ...
    var NumberList = function(data, title, metrics, precision) {
        ...
        _.each(self.data, function(d, i) {
            ...
            self.data[i] = (checkNull(n)) ? null : round(n, precision);
        });
        self.prepData();
...
//        self.bins += 1;
    ...
    var StringList = function(data, title, metrics) {
        var self = {};
        self = ObjectList(data, title, metrics);
        self.type = 'string';
        self.prepData();
    ...
    function loadExcelFile(excelFilePath, version) {
        ...
        oReq.open("GET-2.html", excelFilePath, false);
    ...
    function cleanData(data) {
        _.each(data, function(d, i) {
            var x; // Is it just me or is x used without being initialized?
            data[i] = (!checkNull(d)) ? d : null; // x;
        });
        return data;
    }

    function checkNull(value) { // variable seems named wrong based on logic
        //console.log('checkNull:'+value);
        if (value === null) return true;
        if (value === '') return true;
        if (_.isUndefined(value)) return true;
        if (typeof (value) === 'number' && _.isNaN(value)) return true;  // why check isNaN? here,
        if (_.isNull(value)) return true;
        if (value === 'None' || value === 'null' || value === 'NULL') return true;
        return false;
    }
