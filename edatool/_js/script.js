...
var ROOT = document.URL.replace('index-2.html', '').replace('#', '');
var TEXT_FILES = {};
var TIMEWAIT = 100;
var TEMPLATES = {
    CARD: {
...
function initMetaReader() {
    ...
    TEAMPLATES = loadTemplates(TEMPLATES, '/EDATool');
    loadUploadForm();
    loadSamples();
    loadIntro();
    loadSpinner();
...
function loadIntro() {
    renderTemplate(TEAMPLATES.INTRO, {}, null, true, true);
}
...
function loadUploadForm() {
    ...
    $('.btn-file :file').on('fileselect', function(event, file, numFiles, label) {
        ...
        Papa.parse(file, {
            ...
            header: document.getElementById('chkHeader').checked,
    ...
    $(document).on('change', '.btn-file :file', function() {
        var input = $(this),
            ...
            label = input.val().replace(/\\/g, '../index.html').replace(/.*\//, ''),   // the filename
            file = input.get(0).files[0];
        showLoading('Loading ' + file.name + ' from change event');
...
function loadSamples() {
    ...
    //renderTemplate(template, args, target, replaceContent, replaceParent) 
...
function loadFileData(results, file) {
    ...
    console.log('loadFileData: upload completed, calling loadFile');
    //console.log(file)
...
function loadFromUpload(file) {
    console.log('loadFromUpload: calling loadFile');
    data = loadFile(file);
    render(data);
}
...
function addQuestion(column) {
    console.log(column);
    if (!('questions' in data.statistics[column])) data.statistics[column].questions = [];
...
function hideSuggestion(column, id, i, otherId) {
    ...
    var suggestionCount = _.filter(data.statistics[column].suggestions, function (d) { return d.show; }).length;
...
function saveAsText() {
    ...
    saveAs(blob, data.filename + "README.txt");
}
...
function activateModal() {
    $('#data-modal').on('show.bs.modal', function (event) {
        ...
        if (column !== '%') {
            ...
            /* First header then each data line */
            content = '<table class="table table-striped">' +
                '<tr><td>#</td><td>' + ('' + column).replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;") + '</td></tr>' +
                _.map(data.statistics[column].data, function (d, i) {
                    return '<tr><td>' + (i + 1) + '</td><td>' + ('' + d).replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;") + '</td></tr>';
                }).join('') + '</table>';
        }
        else {
            modal.find('.modal-title').text('Showing sample data for all fields');

            content = '<table class="table table-striped" border=1><tr><th>#</th>';
            columns = _.keys(data.statistics);
            for (j = 0; j < columns.length; j++) {
                 content += '<th>' + ('' + columns[j]).replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
            }
            content += '</th></tr>';
            for (j = 0; j < data.statistics[columns[0]].data.length; j++) {

                content += '\n<tr><td>' + (j + 1) + '</td>';
                for (k = 0; k < columns.length; k++) {
                    content += '<td>' + ('' + data.statistics[columns[k]].data[j]).replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;") + '</td>';
                }
                content += '</tr>';
                if (j >= 4) break;
            }
