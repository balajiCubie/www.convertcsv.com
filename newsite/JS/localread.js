// Copyright 2013 - 2022 Data Design Group, Inc  All Rights Reserved
function ieReadLocalFile(that,callback,encoding) {
        ...
        request.open('get-2.html', fn, true);
...
function readExcelFile(event,callback,fn,sheet) {
    ...
    if(!fn && document.getElementById('f1'))fn=document.getElementById('f1').value.split(/[\\\/]/)[document.getElementById('f1').value.split(/[\\\/]/).length-1];
    if(!fn)fn="convertcsv";
    var ext="XLSX";
...
function loadTextFile(f,callback,event)
{
    var fn='';
    if(f.id)fn=f.id;
    else if(f.name)fn=f.name;
    else if(document.getElementById('f1'))
       fn=document.getElementById('f1').value.split(/[\\\/]/)[document.getElementById('f1').value.split(/[\\\/]/).length-1];
    else fn="convertcsv";
    var encoding = "";
    var sheetname = "";
