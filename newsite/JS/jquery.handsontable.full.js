...
(function (global) {
  ...
  global.SheetClip = {
    ...
    parse: function (str) {  // the orginal
      ...
      detect.caret = countChar(rows[0],'^');
      if(rows.length>1) { // swap delimiter to comma if no tabs and a comma.
         if     (detect.tab >=detect.comma && detect.tab >=detect.pipe && detect.tab >=detect.semi && detect.tab>=detect.colon && detect.tab>=detect.caret) delim = "\t";
         else if(detect.semi >detect.comma && detect.semi >detect.pipe && detect.semi >detect.tab  && detect.semi>detect.colon && detect.semi>detect.caret) delim = ";";
         else if(detect.colon>detect.comma && detect.colon>detect.pipe && detect.colon>detect.tab  && detect.colon>detect.semi && detect.colon>detect.caret) delim = ":";
         else if(detect.pipe >detect.comma && detect.pipe >detect.semi && detect.pipe >detect.tab  && detect.pipe>detect.colon && detect.pipe>detect.caret) delim = "|";
         else if(detect.caret >detect.comma && detect.caret >detect.semi && detect.caret >detect.tab  && detect.caret>detect.colon && detect.caret>detect.pipe) delim = "^";
         else if(detect.comma>detect.tab   && detect.comma>detect.pipe && detect.comma>detect.semi && detect.comma>detect.colon && detect.comma>detect.caret)delim = ",";
         if (detect.tab == 0 && detect.comma == 0 && detect.pipe == 0 && detect.colon == 0 && detect.semi == 0 && detect.caret == 0 && detect.space > 0)delim = " ";
      }
...
(function (jsonpatch) {
    ...
    function apply(tree, patches) {
        ...
        while (p < plen) {
            ...
            // Find the object
            var keys = patch.path.split('../index.html');
            var obj = tree;
            var t = 1;
            var len = keys.length;
            while (true) {
                if (_isArray(obj)) {
                    ...
                    var key = keys[t];
                    if (key.indexOf('~') != -1)
                        key = key.replace(/~1/g, '../index.html').replace(/~0/g, '~');
                    t++;
                    if (t >= len) {
